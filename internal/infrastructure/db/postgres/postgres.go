...
func (db *DB) ApplyMigrations() error {
	logger.Log.Infow("Applying migrations...")
	...
	switch err {
	case nil:
		logger.Log.Infow("Migrations successfully applied")
	case migrate.ErrNoChange:
		logger.Log.Infow("No migrations for applying")
	default:
...
func connect(cfg *Config) (*pgxpool.Pool, error) {
	logger.Log.Infow("Conntecting to Postgres DB...")
